...
This script tabkes one command line argument: the name of a log file
...
def dictify_logline(line):
    ...
    if m:
        groupdic = m.groupdict()
        if groupdic['bytes_sent'] == '-':
            groupdic['bytes_sent'] = '0'
        return groupdic
    else:
...
def generate_log_report(logfile):
    """
    return a dictionary of formant remote_host=>[list of bytes sent]
    This functin takes a file object, iterates through all the lines in the file,
    and generates a report of the number of bytes trasferred to each remote host
